...
# 7. Riduzione della dimensionalità con PCA
pca = PCA(n_components=2)
data_reduced = pca.fit_transform(HADES)

# 8. Applicazione del K-means con n_clusters=2 sui dati ridotti con PCA
KMC = KMeans(n_clusters=2)
KMC.fit(data_reduced)
center_age = [KMC.cluster_centers_[0][0], KMC.cluster_centers_[1][0]]
center_thalachh = [KMC.cluster_centers_[0][1], KMC.cluster_centers_[1][1]]

# 9. Visualizzazione della previsione (n_clusters=2) rispetto ai dati reali sui dati ridotti
plt.figure(figsize=(20,10))
plt.subplot(1, 2, 1)
sns.scatterplot(x=data_reduced[:, 0], y=data_reduced[:, 1], hue=KMC.labels_, s=40)
plt.title("Predicted outcome based on clustering")
plt.xlabel("Principal Component 1")
plt.ylabel("Principal Component 2")

plt.subplot(1, 2, 2)
sns.scatterplot(x=data_reduced[:, 0], y=data_reduced[:, 1], hue=HAD["output"], s=40)
plt.title("Actual outcome")
plt.xlabel("Principal Component 1")
plt.ylabel("Principal Component 2")
plt.savefig("grafici_cluster_PCA/Previsione Cluster.png")
plt.show()
...
# 11. Metodo del gomito per determinare il numero ottimale di cluster
metric = []
...
plt.savefig("grafici_cluster_PCA/Elbow Method Cluster.png")
plt.show()

# 12. Analisi silhouette per diverse quantità di cluster
def silhouette_analysis(range_n_clusters, X):
    for n_clusters in range_n_clusters:
        ...
        plt.savefig(f'grafici_cluster_PCA/Grafico_{n_clusters}.png')
        plt.show()

# 13. Applicazione dell'analisi silhouette sui dati ridotti
