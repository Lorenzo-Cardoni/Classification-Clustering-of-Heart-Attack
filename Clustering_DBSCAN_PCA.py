...
def elbow_method(dataset, n_knn, config):
    ...
    # Individua il ginocchio (knee) utilizzando KneeLocator
    ...
    plt.ylabel("Distance")
    plt.legend()
    plt.savefig(f"grafici_DBSCAN/elbow_dbscan_{config}_PCA.png", bbox_inches='tight')
    plt.show()
...
def select_parameter(eps, dataset, config):
    ...
    plt.tight_layout()
    plt.savefig(f"grafici_DBSCAN/parametri_dbscan_{config}_PCA.png", bbox_inches='tight')
    plt.show()
...
df.to_csv('metrics_DBSCAN.csv', header=True, index=False, mode='a')
...
plt.legend()
plt.savefig("grafici_DBSCAN/risultato_dbscan_PCA.png", bbox_inches='tight')
plt.show()


print("questo codice usa DBSCAN con i dati standard e la visualizzazione con PCA2")